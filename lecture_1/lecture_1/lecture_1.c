...
#include <locale.h> // поддержка языка, в данном примере для вывода на русском
#include <stdio.h> // стандартный ввод/вывод
#include <conio.h> // в данном примере для _getch()
#include <inttypes.h> //в данном случае для вывода min и max значений в типах данных
#include <windows.h>
...
int main() {
  ...
  //переполнение
   ...
   printf("Переполнение short: %hd\n", s_short); // 32767
   s_short = 32768; 
   printf("Переполнение short: %hd\n", s_short); // -32768
   s_short = 32790;
   printf("Переполнение short: %hd\n", s_short); // -32746
   unsigned c_char_over = 255;
   printf("Переполнение беззнакового char: %hhu\n", c_char_over); // 255
   c_char_over = 261;
   printf("Переполнение беззнакового char: %hhu\n", c_char_over); // 5
  ...
  p = pReal; // p получает значение 2, неявное
  pReal = p; // pReal теперь равно 2.0  
  int p1;
  double pReal1 = 2.718281828;
  printf("\npReal1=%lf ", pReal1);
  p1 = (int)pReal; // p получает значение 2, явное
  ...
  char c_char;
  printf("Input c_char= ");
  scanf(" %c", &c_char); //пробел перед %c проигнорирует enter от прошлого ввода, иначе в c_char запишется '\n'
  printf("i_int=%i, f_float=%f, d_double=%0.1lf, c_char=%c \n", i_int, f_float, d_double, c_char);// 0.1lf - выведется один знак после запятой
